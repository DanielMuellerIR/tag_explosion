...
#
# Geprüft wird genau das, was ein Nutzer im Finder tut und was zuvor kaputt war
# (Befund 2026-08-17): Datei öffnen, Fenster schließen, wieder öffnen. Dazu
# Seitenleiste, Dateiname und Datei-Icon im Fensterkopf.
#
...
[ "$#" -ge 3 ] || {
    echo "Aufruf: dev-windowtest.sh <app> <ausgabeordner> <datei1> [datei2]" >&2
    exit 2
}
...
// ---- Rumpf: Fenster öffnen, schließen, wieder öffnen -------------------------

let documents = testArguments.map { URL(fileURLWithPath: $0) }
guard !documents.isEmpty else { kitFail("mindestens eine Testdatei angeben") }
...
guard NSRunningApplication.runningApplications(withBundleIdentifier: testBundleID).isEmpty else {
    kitFail("Es läuft bereits eine Tag Explosion — der Test würde sie stören.")
}

guard let running = launchTestInstance(documents: Array(documents.prefix(1))) else {
...
func screenshot(_ name: String) {
    let problem = captureWindow(of: pid, to: outDir.appendingPathComponent(name))
    check(problem == nil, problem ?? "Belegbild \(name) ist frisch entstanden und nicht leer")
}
...
_ = launchTestInstance(documents: Array(documents.prefix(1)), newInstance: false)
...
if documents.count > 1 {
    _ = launchTestInstance(documents: [documents[1]], newInstance: false)
    Thread.sleep(forTimeInterval: 3.0)
