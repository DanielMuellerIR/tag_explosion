...
- Groß-/Kleinschreibungs-Werkzeuge
...
- GUI-Tests: Maus-Klicks via CGEvent funktionieren, synthetische Tastatur-Events
  ...
  Accessibility-API (`scripts/dev-uitest.sh`) und speichern über den Menüpunkt.
- Fensterverhalten (öffnen, schließen, wieder öffnen, Seitenleiste, Datei-Icon
  im Titel) prüft `scripts/dev-windowtest.sh` ebenfalls über die
