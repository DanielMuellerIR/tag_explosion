...
| AP3 | **Umbenennen aus Tags und Tags aus Dateinamen** mit Format-Mustern (`%artist% - %title%`, `%track%`, `%album%` …). Muster-Engine im Core, Vorschau vor dem Umbenennen, Kollisionen erkennen, `tagx rename` und `tagx parse` (dry-run per Voreinstellung), Batch-Editor-Menü in der App. | groß | 🔧 (Zweig `wp/ap3-rename`, Merge ausstehend) |
